...
function req(body: unknown, bearer = "valid-token") {
  return new Request("http://localhost/api/subscriptions/submit-otp", {
    method: "POST",
...
beforeEach(() => {
  ...
  maybeSingleMock.mockResolvedValue({ data: { owner_id: "user_1", status: "pending", otp_submit_attempts: 0 }, error: null });
...
describe("POST /api/subscriptions/submit-otp", () => {
  ...
  it("returns not_found rather than relaying an OTP for a reference belonging to a different owner", async () => {
    maybeSingleMock.mockResolvedValueOnce({ data: { owner_id: "someone_else", status: "pending", otp_submit_attempts: 0 }, error: null });
    const res = await POST(req({ reference: "ref_1", otp: "123456" }));
  ...
  it("skips Paystack and reports ok: false when the payment is no longer pending", async () => {
    maybeSingleMock.mockResolvedValueOnce({ data: { owner_id: "user_1", status: "success", otp_submit_attempts: 0 }, error: null });
    const res = await POST(req({ reference: "ref_1", otp: "123456" }));
    expect(await res.json()).toEqual({ ok: false, message: "This payment is no longer waiting for a code." });
    expect(submitPaystackOtpMock).not.toHaveBeenCalled();
  });

  it("relays the OTP to Paystack and returns its result for a pending, owned payment", async () => {
    const res = await POST(req({ reference: "ref_1", otp: "123456" }));
  ...
  // Regression coverage: this route had no attempt limit at all before —
  // an adversarial review found it could be hammered indefinitely, unlike
  // the sibling SMS-OTP route (OTP_MAX_VERIFY_ATTEMPTS).
  ...
  it("locks out and fails the payment once the attempt cap is reached, without calling Paystack again", async () => {
    maybeSingleMock.mockResolvedValueOnce({ data: { owner_id: "user_1", status: "pending", otp_submit_attempts: 5 }, error: null });
    const res = await POST(req({ reference: "ref_1", otp: "123456" }));
    expect(submitPaystackOtpMock).not.toHaveBeenCalled();
