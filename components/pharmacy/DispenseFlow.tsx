...
interface DispenseFlowProps {
  id: string;
  backHref: string;
  /** Builds "Back to {backTarget}" / "Go to {backTarget}" — "review" for a pharmacist (back to /pharmacist/review/[id]), "prescription" for an independent physician self-dispensing (back to their own /prescriptions/[id]). */
  backTarget: string;
}

/**
 * The real dispense gate: batch/quantity selection (FEFO-suggested),
 * mandatory override notes on flagged lines, and the atomic
 * screen-then-decrement-then-record write via POST /api/pharmacy/dispense.
 * Extracted from app/(app)/pharmacist/dispense/[id]/page.tsx so both a
 * pharmacist and an independent physician dispensing their own prescription
 * (0033_independent_physician_self_service.sql) share one implementation —
 * the route enforces who may act on which prescription; this component just
 * needs to know where "back" goes. actorRole on the logged dispense action
 * is derived from the real signed-in role, never passed in, so it can't
 * drift from who's actually dispensing.
 */
export function DispenseFlow({ id, backHref, backTarget }: DispenseFlowProps) {
  const { user, role } = useAuth();
  const showToast = useToastStore((s) => s.show);
  const actorRole = role === "prescriber" ? "prescriber" : "pharmacist";
  ...
  if (!finalized && state !== "cleared") {
    return (
      <PageShell maxWidth="2xl" className="space-y-4">
        <Link href={backHref} className="inline-flex items-center gap-1.5 text-sm font-medium text-brand hover:underline">
        ...
        <Card>
          <CardBody className="space-y-2 text-center">
            ...
            <p className="text-sm text-muted-foreground">This prescription must be Approved before it can be dispensed.</p>
            <Link href={backHref}>
              <Button className="mt-2">Go to {backTarget}</Button>
            </Link>
          </CardBody>
        </Card>
      </PageShell>
  ...
  async function handleFinalize() {
    ...
    await appendAction.mutateAsync({ prescriptionId: id, pharmacistId: user.id, action: "dispense", actorRole });
    // No separate status update here — dispense_drug() now flips
    // prescriptions.status to "dispensed" itself, atomically, as of
    // 0015_close_raw_dispense_status_bypass.sql. Neither a pharmacist's nor
    // an independent physician's own RLS-gated session can set that status
    // via a raw update at all.
