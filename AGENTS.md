...
- **`find_pending_check_payment` (`supabase/migrations/0031_find_pending_check_payment.sql`) grants `anon` a phone-number → live-Paystack-reference lookup with no proof the caller ever verified that phone themselves** — it only checks `check_payments.phone = normalize_gh_phone(p_phone)` and `status = 'pending'`, matching this subsystem's existing "the phone number itself is the credential" trust model (same as `get_check_quota`, `create_patient_check_with_quota`), but that model was designed around low-stakes reads, not handing out a reference an unauthenticated caller can then throw OTP guesses at via `/api/payments/submit-otp`. Found via adversarial review while building the Paystack Mobile Money OTP relay (`lib/payments/paystackCharge.ts`) — `submit-otp` itself is now capped at `OTP_MAX_SUBMIT_ATTEMPTS = 5` (mirroring `app/api/otp/verify/route.ts`'s `OTP_MAX_VERIFY_ATTEMPTS`), which bounds brute-force damage per reference, but does nothing about the reference being obtainable in the first place from just a phone number. Deliberately NOT fixed here — closing it means redesigning the self-check subsystem's whole anonymous-trust model (an actual session/possession proof, not just "knows the phone number"), which touches multiple existing RPCs beyond the scope of the Mobile Money fix that found it. Needs its own deliberate look.
